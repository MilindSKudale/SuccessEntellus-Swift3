...
class MultiSelecetionViewController: UIViewController,UIGestureRecognizerDelegate,UISearchBarDelegate {
    ...
    open var selectedItems : [SwiftMultiSelectItem] = [SwiftMultiSelectItem](){
        didSet{
            ...
            rightButtonBar.image = #imageLiteral(resourceName: "ic_import")
    ...
    var SelectButtonBar  : UIBarButtonItem = UIBarButtonItem()
    ...
    /// Selector for right button Select all
    @objc public func selectAllContacts(){
        
        if SelectButtonBar.title == "Select All"{
            self.selectedItems.removeAll()
            for item in SwiftMultiSelect.items!{
                
                self.selectedItems.append(item)
            }
            SelectButtonBar.title = "Deselect All"
            
        }else{
            SelectButtonBar.title = "Select All"
            self.selectedItems.removeAll()
        }
    ...
    override func viewDidLoad() {
        ...
        SelectButtonBar.title = "Select All"
        SelectButtonBar.isEnabled               = true
        SelectButtonBar.action = #selector(MultiSelecetionViewController.selectAllContacts)
        SelectButtonBar.target = self
        ...
        leftButtonBar?.image = #imageLiteral(resourceName: "ic_blk_Close")
        self.navigationItem.leftBarButtonItem   = leftButtonBar
        self.navigationItem.setRightBarButtonItems([rightButtonBar,SelectButtonBar], animated: true)
