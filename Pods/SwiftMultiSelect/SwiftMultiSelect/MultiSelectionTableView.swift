...
extension MultiSelecetionViewController : UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if SwiftMultiSelect.dataSourceType == .phone{
            if searchString == "" {
                let cnt = SwiftMultiSelect.items?.count ?? 0
                return cnt
            }else{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if selectedItems.count > 0 {
            if selectedItems.containsReference(obj: SwiftMultiSelect.items![indexPath.row] as AnyObject){
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if cell.accessoryType == UITableViewCellAccessoryType.checkmark
        {
            ...
            cell.accessoryType = UITableViewCellAccessoryType.none
            if selectedItems.count > 0 {
                if selectedItems.containsReference(obj: item as AnyObject) {
...
//                }
            ...
            if selectedItems.count < (SwiftMultiSelect.items?.count)! {
                self.SelectButtonBar.title = "Select All"
            }else if selectedItems.count == (SwiftMultiSelect.items?.count)! {
                self.SelectButtonBar.title = "Select All"
            }else{
                self.SelectButtonBar.title = "Deselect All"
            }
        }
        else{
            //Set accessory type
