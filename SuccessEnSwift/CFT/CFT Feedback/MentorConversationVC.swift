...
class MentorConversationVC: JSQMessagesViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIDocumentPickerDelegate, UIDocumentMenuDelegate {
    ...
    override func viewDidLoad() {
        ...
        // self.collectionView?.frame.size.height = self.view.frame.height - 30
    ...
    @objc func backButtonTapped() {
        NotificationCenter.default.post(name: Notification.Name("UpdateUIMentor"), object: nil)
    ...
    @objc func receiveMessagePressed(_ sender: UIBarButtonItem) {
        ...
        if (copyMessage == nil) {
            copyMessage = JSQMessage(senderId: cftAccessUserId, displayName: userName, text: "First received!")
        }
        ...
        if (copyMessage! as AnyObject).isMediaMessage() {
            ...
            newMessage = JSQMessage(senderId: cftAccessUserId, displayName: userName, media: newMediaData)
        }
        else {
            /**
            ...
            newMessage = JSQMessage(senderId: cftAccessUserId, displayName: userName, text: (copyMessage! as AnyObject).text)
        }
...
//            importMenu.me .mediaTypes = [kUTTypeImage as String]
...
extension MentorConversationVC  {
