...
extension UIViewController {
    ...
    public func presentPopup(controller:UIViewController,completion: ((Bool)->())?){
        ...
        containerView.frame = CGRect(x: 0, y: 64, width: self.view.frame.width, height: self.view.frame.height)
        //UIView(frame: mainView.bounds)
    ...
    public func dismissPopup(completion: ((Bool)->())?){
        setupDismissAnimation(type: .bottom)
    }
}
