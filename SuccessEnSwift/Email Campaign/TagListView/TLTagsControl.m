...
@synthesize tapDelegate, deleteTagButton;
...
- (void)commonInit {
    ...
    tagInputField_.layer.borderColor = [UIColor blackColor].CGColor;
...
- (void)reloadTagSubviews {
    ...
    for (NSString *tag in _tags) {
        ...
        if (_mode == TLTagsControlModeEdit) {
            deleteTagButton = [[UIButton alloc] initWithFrame:tagInputField_.frame];
           ...
           // [deleteTagButton addTarget:self action:@selector(deleteTagButton:) forControlEvents:UIControlEventTouchUpInside];
            ...
            buttonFrame.size.height = tagInputField_.frame.size.height;
            [deleteTagButton setTag:tagSubviews_.count];
            if (_tagsBackgroundColor == [UIColor whiteColor]) {
                if (![tag isEqualToString: @"No campaigns assigned yet."]){
                    [deleteTagButton setImage:[UIImage imageNamed:@"campain_black"] forState:UIControlStateNormal];
                }else{
                    [deleteTagButton setImage:nil forState:UIControlStateNormal];
                }
                
            }else{
                [deleteTagButton setImage:[UIImage imageNamed:@"ic_file"] forState:UIControlStateNormal];
            }
           // [//deleteTagButton setImage:_image forState:UIControlStateNormal];
            //[deleteTagButton setTitleColor:tagDeleteButtonColor forState:UIControlStateNormal];
            buttonFrame.origin.y = 0;
            ...
            tagFrame.size.width = labelFrame.size.width + buttonFrame.size.width;
            [tagView addSubview:deleteTagButton];
            labelFrame.origin.x = buttonFrame.size.width;
        } else {
